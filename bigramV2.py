# Keep getting errors on this one
...
n = int(0.9 * len(data))  # first 90% of data will be used for training rest validation
train_data = data[:n]
val_data = data[n:]
...
class Block(nn.Module):
    ...
    def __init__(self, n_embd, n_head):
        # n_embd: embedding dimensions, n_head: the number of heads used for attention
        super().__init__()
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        pos_emb = self.position_embedding_table(torch.arange(T, device=device, dtype=torch.long))  # (T, C)
